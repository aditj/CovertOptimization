...
def get_cost(oracle_state,learner_state,eavesdropper_estimate,action,num_queries):
    ...
    cost = 0
    if action == 0:
        cost =  ((oracle_state+1)**2/((learner_state+1)**2)) * np.log((num_queries*eavesdropper_estimate)/((num_queries+1)*eavesdropper_estimate))
    else:
        cost = ((learner_state+1)**2)/(oracle_state+1)**(2)* np.log((num_queries*eavesdropper_estimate+1)/((num_queries+1)*(eavesdropper_estimate)))*1/(1+np.exp((-eavesdropper_estimate+0.5)/0.01))
...
# print(get_approx_cost(P_O,lambda x: sigmoid_policy(x,[0,0]),O,M,U,N_rounds,1))
# exit()
