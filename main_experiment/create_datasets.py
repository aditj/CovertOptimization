...
##    df = df.sample(frac=fraction_of_data).reset_index(drop=True)
    ...
    if create_eavesdropper:
        ...
        valid_df_eav = pd.concat([valid_df_eav_1,valid_df_eav_0])
        valid_df_eav = valid_df_eav.reset_index(drop=True).sample(n = int(N_batch*2))
        ...
        train_df_eav.to_csv(f"./data/client_datasets/client_1000.csv",index=False)
        valid_df_eav.to_csv(f"./data/client_datasets/client_2000.csv",index=False)